...
| UNIT-06 | Unit | AC-19, AC-20, AC-21 | Content trim/1-5000 bounds and projection separation; script-like text is data | server/tests/lab-03/conversations.test.ts | Passed locally 2026-09-20 (11 tests): trim and 1/5000 JS string-unit boundaries, empty/whitespace/non-string/5001 rejection, emoji semantics, exact internal whitespace/newline and script-like data preservation, unknown/backend-owned field rejection, and separate exact safe Comment/Note Entry projections. Hosted Server CI passed on exact implementation HEAD `b17c9a2` in run `35458345019`. |
...
| API-06 | API | AC-06 | Parameterize every protected endpoint/method against every role, including restricted/anonymous; matrix denial precedes resource lookup | server/tests/lab-03/authorization.api.test.ts | Existing Issue #55/#60 coverage remains; Issue #61 locally replaces the temporary `/notes` stub with real `/comments` and `/internal-notes` GET/POST matrix cases for anonymous, restricted, Requester, IT Staff and Administrator, including Staff/Admin Internal Note success and role-first Requester denial. The same matrix passed on exact implementation HEAD `b17c9a2` in Server CI run `35458345019`. |
...
| API-08 | API | AC-20 | Requester Note GET/POST403 even for missing parents; no note content/author/count through detail/comments/errors | server/tests/lab-03/comments-notes.api.test.ts | Passed locally 2026-09-20: Requester GET/POST and unsupported methods return role-first 403 before ID/query/body/CSRF/parent/note lookup, including missing parents; mock API assertions prove no Note content, author, count or fragment in Requester Ticket detail, Public Comments or safe errors. Hosted Server CI passed on exact implementation HEAD `b17c9a2` in run `35458345019`. |
...
| API-27 | API/PostgreSQL | AC-19 | Public content bounds, exact author/time, chronological order and empty list; one valid POST creates one entry201, unsupported clientRequestId400 | server/tests/lab-03/comments-notes.api.test.ts, server/tests/lab-03/comments-notes.postgres.integration.test.ts | Mock API coverage passed locally 2026-09-20: own Requester/Staff/Admin GET/POST, exact chronological/empty DTOs, backend author/time, trim and 1/5000 bounds, client metadata rejection, terminal access, monotonic `updatedAt`-only touch, explicit GET-body rejection, and two independent POST rows. JavaScript string-unit/emoji semantics pass in UNIT-06. The 5-test real PostgreSQL suite was skipped locally because isolated `TEST_DATABASE_URL` and `RUN_DB_INTEGRATION=1` were not configured; no skip is claimed as a local pass. All 5 tests then passed on exact implementation HEAD `b17c9a2` in Server CI run `35458345019`, within the full 28-file / 212-test pass. |
| API-28 | API/PostgreSQL | AC-20 | Staff/Admin private append/read on terminal Tickets; edit/delete405; no cross-parent leak or Requester note projection | server/tests/lab-03/comments-notes.api.test.ts, server/tests/lab-03/comments-notes.postgres.integration.test.ts | Mock API coverage passed locally 2026-09-20 for Staff/Admin ordered append/read on CLOSED/CANCELLED Tickets, separate Comment/Note models/projections, Requester role-first denial, and PUT/PATCH/DELETE safe 405 with `Allow: GET, POST` and no mutation/leak. The guarded 5-test PostgreSQL suite was skipped locally without an isolated configured test database; hosted CI is pending until push. |
| API-29 | API | AC-22 | Admin list fields/name-email OR search/role AND filter/default order/empty; non-admin403; unknown params400 | server/tests/lab-03/users-admin.api.test.ts | Passed on Issue #63 exact HEAD `ff149aa`; hosted Server CI `36142262536` passed the complete 30-file / 219-test suite. |
| API-30 | API/PostgreSQL | AC-23 | Admin create/edit one role/name/email/Boolean; duplicate normalized/inactive email409 and concurrent duplicates; stale edit409 | server/tests/lab-03/users-admin.api.test.ts; server/tests/lab-03/users-admin.postgres.integration.test.ts | Passed on Issue #63 exact HEAD `ff149aa`; normalized duplicate and stale-version PostgreSQL coverage included in Server CI `36142262536`. |
| API-31 | API/PostgreSQL | AC-15, AC-24 | Deactivate/demote/account edit revokes sessions and unassigns active and CLOSED/CANCELLED owners in one transaction; provenance former owner/actor/time persists; status/resolution/Requester/authorship/removal links unchanged | server/tests/lab-03/users-admin.api.test.ts; server/tests/lab-03/users-admin.postgres.integration.test.ts | Passed on Issue #63 exact HEAD `ff149aa`, including session revocation and ACCOUNT_INELIGIBLE TicketOwnerChange provenance in hosted Server CI `36142262536`. |
| API-32 | API/PostgreSQL | AC-25 | Self-deactivation409; last active admin demotion/deactivation409; simultaneous changes cannot eliminate admins | server/tests/lab-03/users-admin.api.test.ts; server/tests/lab-03/users-admin.postgres.integration.test.ts | Passed on Issue #63 exact HEAD `ff149aa`, including real concurrent last-active-admin serialization in hosted Server CI `36142262536`. |
| API-33 | API/PostgreSQL | AC-26 | Reset validates/salts/hash/flag/revocation/version; no response password; self reset returns reauthenticationRequired | server/tests/lab-03/users-admin.api.test.ts | Passed on Issue #63 exact HEAD `ff149aa`; hosted Server CI `36142262536`. |
| API-34 | API | AC-28 | Invalid JSON/bounded payloads and injected DB/file/service failures use exact safe errors/correlationId; no sensitive fragments | server/tests/lab-03/auth.api.test.ts, server/tests/lab-03/auth.review-regression.api.test.ts, server/tests/lab-03/comments-notes.api.test.ts | Existing auth malformed JSON/payload-boundary evidence remains. Issue #61 injected Comment/Note list/create failures passed locally 2026-09-20 and on exact implementation HEAD `b17c9a2` in Server CI run `35458345019`, with exact generic `INTERNAL_ERROR`, correlationId, and no submitted content, private Note fragment or SQL text. Other non-auth scopes remain pending, so full API-34 is not claimed complete. |
...
| DB-04 | Concurrency | AC-15, AC-24, AC-25 | Owner assignment racing with account demotion cannot persist an ineligible owner; terminal owner cleanup retains TicketOwnerChange under concurrency; last-admin safety uses real transactions | server/tests/lab-03/staff-workflow.postgres.integration.test.ts, server/tests/lab-03/users-admin.api.test.ts | #60 assignment side passed on real PostgreSQL in exact implementation HEAD `112e226` Server CI run `35454755155`: the shared account-safety advisory lock serialized an eligibility change and the assignment recheck rejected the now-ineligible target without owner/event mutation. #63 demotion/deactivation cleanup/provenance and last-admin transaction serialization passed on exact HEAD `ff149aa` in hosted Server CI `36142262536`; together with the earlier assignment-side race this completes DB-04 automated evidence. |
...
| UI-06 | UI | AC-14, AC-15, AC-16, AC-17 | Detail read-only values vs operational controls; claim/assign/reassign but no manual-unassign action; confirmations/matrix fields/save busy/409 refresh and terminal rules | client/tests/lab-03/StaffTicketDetail.test.tsx | Passed on merged Issue #75: 10 focused tests; included in current 93/93 client regression. |
| UI-07 | UI | AC-18, AC-19, AC-20, AC-21 | Requester indication/comment flow; public/private independent drafts/buttons; inert scripts; ambiguous-post list reload and review-before-manual-retry; no requester notes | client/tests/lab-03/TicketConversation.test.tsx; client/tests/lab-02/RequesterTicketDetail.test.tsx | Passed on merged Issue #75: 5 focused conversation tests plus Requester indication/no-Internal-Notes regression; included in current 93/93 client suite. |
| UI-08 | UI | AC-22, AC-23, AC-25 | Admin list/create/edit/search/role/activation, duplicate/one-role/last-admin/self/stale errors and retry | client/tests/lab-03/UserManagement.test.tsx | Passed on Issue #63 exact HEAD `ff149aa`; hosted Client CI `36142262695` passed 15 files / 93 tests. |
| UI-09 | UI | AC-24, AC-26, AC-28 | Reset/account edit confirmations, cleared passwords, self reauthentication and safe processing/failure feedback | client/tests/lab-03/UserManagement.test.tsx | Passed on Issue #63 exact HEAD `ff149aa`; hosted Client CI `36142262695`. |
...
### Issue #60 local and hosted execution record - 2026-09-19
...
| Hosted Client CI | Exact implementation HEAD `112e226`: run `35454755168` passed 12 files / 75 tests. |
| Hosted Server CI | Exact implementation HEAD `112e226`: run `35454755155` passed 25 files / 169 tests, including all 4 Issue #60 PostgreSQL integration/concurrency tests. |
| Hosted E2E CI | Exact implementation HEAD `112e226`: run `35454755178` passed 16/16 Playwright tests. No full E2E-03 claim is made because #61/#75 remain pending. |

The local mock/API evidence covers UNIT-03 and API-21 through API-25. The guarded PostgreSQL suite was skipped locally, then all four Issue #60 PostgreSQL tests passed in hosted Server CI `35454755155`, covering owner-event atomicity, same-version owner/priority/status competition, real timestamp/reset persistence and the #60 shared-lock eligibility-recheck side. Full DB-04 remains incomplete until #63 supplies and tests real Administrator demotion/deactivation cleanup plus last-admin serialization. UI-06, E2E-03 and Lab 3 completion remain unclaimed.

### Issue #61 local execution record - 2026-09-20

Issue #61 implements only the Public Comments and restricted Internal Notes APIs under umbrella #58. It adds separate Prisma services/selects/projections, append-only GET/POST routes, exact Entry DTOs, ownership/role enforcement, workflow-neutral `Ticket.updatedAt` touches, safe unsupported-method handling and failure redaction. It does not add #75 integrated Staff/Requester conversation UI or #63 Administrator APIs, and it does not change any migration.

| Command | Observed result |
| --- | --- |
| `git diff --check` | Passed locally; no whitespace errors. |
| `npm --prefix server test` | Passed runnable suites: 18 files / 187 tests. Skipped: 10 PostgreSQL integration files / 25 tests because this local run did not configure the required isolated `TEST_DATABASE_URL` and `RUN_DB_INTEGRATION=1`; this includes all 5 new Issue #61 PostgreSQL tests, and no skip is claimed as a pass. |
| `npm --prefix server run build` | Passed: TypeScript build. |
| `npm --prefix client test` | Passed unchanged client regression: 12 files / 75 tests; 0 failed/skipped. |
| `npm --prefix client run build` | Passed: TypeScript and Vite production build. |
| `cd client && npx playwright test --list` | Passed discovery: 16 tests in 6 files. No Issue #61 browser scenario is claimed because conversation UI/E2E integration remains #75 and E2E-02/E2E-03 remain incomplete. |
| Hosted Client/Server/E2E CI | Exact implementation HEAD `b17c9a2`: Client CI `35458345014` passed 12 files / 75 tests; Server CI `35458345019` passed 28 files / 212 tests including all 5 Issue #61 PostgreSQL tests; E2E CI `35458345053` passed 16/16 existing Playwright tests. No full E2E-02/E2E-03 claim is made because integrated conversation UI/browser flow remains #75. |

The local evidence covers UNIT-06 and the Issue #61 portions of API-06, API-08, API-27, API-28 and API-34. The isolated PostgreSQL suite is skipped locally but all 5 real-database cases passed on exact implementation HEAD `b17c9a2` in Server CI `35458345019`. UI-07 conversation behavior, full E2E-02/E2E-03, full API-34, #75, #63 and Lab 3 completion remain unclaimed.

### Current Issue mapping — 2026-09-20
...
| #60 | Ownership, priority and Workflow API | UNIT-03 and API-21-25 passed locally; all 4 guarded PostgreSQL Issue #60 tests passed in exact implementation HEAD `112e226` Server CI `35454755155`; Client CI `35454755168` and E2E CI `35454755178` also passed; full DB-04 still depends on #63 |
| #61 | Public Comments/Internal Notes APIs | UNIT-06 and mock/API portions of API-06/08/27/28/34 passed locally; all 5 guarded PostgreSQL tests passed on exact implementation HEAD `b17c9a2` in Server CI `35458345019`; Client CI `35458345014` and E2E CI `35458345053` also passed; no #75 UI/browser completion is claimed |
| #75 | Staff Detail/UI integration | Merged via PR #81; UI-06/07 and E2E-03 implemented and regression-green; final STYLE/A11Y/manual evidence remains #65. |
| #63 | Administrator User Management | Merged via PR #82; API-29-33, DB-04 admin side, UI-08/09 and E2E-04 passed on exact HEAD `ff149aa` (Client `36142262695`, Server `36142262536`, E2E `36142262756`). |
| #65 | Final integration/release/PDF | RELEASE-01 and final evidence pending |

Issue #56 has reproducible local evidence and exact implementation HEAD `7da6714` hosted evidence: Client CI `35428519145` passed 66/66, Server CI `35428519093` passed 96/96 including PostgreSQL integration, and E2E CI `35428519122` passed 14/14. E2E-01 is therefore evidenced as passed for Issue #56. The Issue #56 identity/resolution and adapted Lab 2 regression portion of E2E-02 passed in that same E2E run, but the full E2E-02 row remains incomplete until #75 integrates the conversation UI/browser flow. Responsive and final manual visual evidence remain pending.
...
### Issue #63 and final-integration baseline - 2026-09-26

PR #82 merged Issue #63 into `lab3-staging` as merge `3bcb4a9`. Exact implementation HEAD `ff149aa` has hosted green evidence: Client CI `36142262695` = 15 files / 93 tests; Server CI `36142262536` = 30 files / 219 tests with the guarded PostgreSQL suites enabled; E2E CI `36142262756` = 20/20 Playwright tests. This supplies the Administrator API/UI/PostgreSQL/E2E evidence referenced above.

Issue #65 started from `lab3-staging` merge `3bcb4a9` on branch `feature/36-lab3-final-integration`. Local baseline on 2026-09-26: client 15 files / 93 tests passed; server 19 runnable files / 191 tests passed and 11 PostgreSQL files / 28 tests skipped because this local shell does not have an isolated `TEST_DATABASE_URL`; server/client production builds passed; Playwright discovery found 20 tests in 8 files; `git diff --check` passed. These local PostgreSQL skips are **not** release passes; the final release must use hosted/isolated evidence with zero required skips.

Remaining #65 gates are final Lab-3-native responsive/visual/accessibility human evidence, documentation/reviewer/AI/reflection synchronization, release PR peer review, and rerunning affected checks on the final `main` revision. RELEASE-01 remains incomplete until those gates are recorded.
