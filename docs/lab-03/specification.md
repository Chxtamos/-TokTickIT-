...
- **FR-02:** A successful initial-password login permits only current-user retrieval, mandatory password change and logout before normal application access.
- **FR-03:** Change password after current-password verification and valid new-password confirmation; record passwordChangedAt, increment User.version/updatedAt, invalidate old sessions and establish a fresh normal session.
...
- **FR-12:** Claim, assign, reassign or unassign one primary Ticket Owner using validated active eligible accounts and concurrency checks.
...
- **BR-02:** `mustChangePassword=true` blocks normal protected endpoints with `403 PASSWORD_CHANGE_REQUIRED`, including reference data. Restricted sessions last at most 15 minutes.
...
- **BR-09:** Use asynchronous Node.js scrypt with N=131072, r=8, p=1, 64-byte output, random 16-byte salt and maxmem=256 MiB. Store a versioned encoding of parameters/salt/hash; use constant-time comparison and a dummy hash for unavailable users. SHA-256 is only for random session-token hashes and existing payload idempotency, not passwords.
...
- **BR-11:** Normal sessions expire after 8 hours absolute or 30 minutes idle, whichever occurs first. Login rotates tokens; logout deletes the current session. Password change/reset deletes all target sessions. No remember-me feature.
- **BR-12:** Session middleware reloads current User activation/role/password-change state on every protected request; database failures fail closed. Account edits revoke target sessions within the account transaction.
...
| Current user; password change; logout | No | Yes | Yes | Yes | Yes |
...
- **BR-18:** Every route checks session, account state and permitted role before parsing/querying protected resources; authenticated Requester resource queries then scope by requesterId. Operational writes are shared-role actions, not restricted to the currently assigned staff member.
...
- **BR-22:** There is at most one primary Ticket Owner. Assignment targets must currently be active IT Staff or Administrator; unassignment uses null. Claim assigns self only when unassigned; claiming a Ticket already owned by self is a no-op; another owner's claim is `409 OWNER_CONFLICT`. Claim does not change status automatically.
- **BR-23:** Owner/priority mutations are rejected on CLOSED/CANCELLED. An inactive or demoted owner is unassigned atomically during the user edit; historical Requester/author/remover FKs remain intact even after role changes.
...
- **BR-28:** The UI confirms every status change, reassignment/unassignment, resolution indication, password reset and activation change. Backend validates independently; confirmations are deliberate UI steps, not client-supplied authorization flags.
...
- **BR-32:** Each conversation create contains a UUID clientRequestId unique per ticket/author/visibility. Same normalized content/key returns the saved entry with `200`, different content/key `409 IDEMPOTENCY_CONFLICT`; first creation is `201`. Lists order createdAt asc then id asc. New entries update Ticket.updatedAt but not Ticket.version.
...
| Session | id UUID PK generated server-side; tokenHash Char(64) unique NOT NULL; userId Int FK User NOT NULL; csrfToken Char(64) NOT NULL (random 32-byte hex); createdAt/lastSeenAt/expiresAt DateTime NOT NULL. Store no raw session token. User state determines restricted access; expiresAt fixes the appropriate absolute duration. |
| LoginThrottle | keyHash Char(64) PK; failureCount Int NOT NULL default 0; windowStartedAt DateTime NOT NULL. A prefix distinguishes IP and email/IP SHA-256 keys; atomic updates apply BR-14; expired buckets reset and can be cleaned up. No account-history feature. |
...
| PublicComment | id Int autoincrement PK; ticketId Int FK Ticket NOT NULL; authorId Int FK User NOT NULL; content VarChar(5000) NOT NULL; clientRequestId UUID NOT NULL; createdAt DateTime NOT NULL default now. Unique(ticketId,authorId,clientRequestId). |
| InternalNote | Same fields/types/constraints as PublicComment in a separate table; separate uniqueness scope. |
...
Enum UserRole has REQUESTER/IT_STAFF/ADMINISTRATOR; extend TicketStatus with the seven added values. Existing RequestedPriority enum is reused for both priorities.

Retain existing owner/filter indexes and normalized-email CHECK. Rename User table constraints/indexes/sequence as needed to match Prisma without resetting ID allocation. Add User(role,isActive), Session(userId) and Session(expiresAt); Ticket(ticketOwnerId,updatedAt,id), Ticket(currentStatus,itPriority,updatedAt,id), Ticket(updatedAt,id), Ticket(requesterResolvedById); each conversation (ticketId,createdAt,id) and (authorId). Foreign keys and uniqueness are database-enforced; active-role/last-admin/transition checks are service-enforced within transactions. Verify index needs against queries rather than adding an unbounded filter-index set.
...
2. Commit new migrations that extend enums, rename RequesterUser to User (not drop/recreate), preserve all FK references/IDs, add the above models/columns, and backfill role=REQUESTER, mustChangePassword=true and IT Priority. Keep original activation. passwordHash starts NULL: no user can login unprovisioned.
...
- **AC-02:** Initial-password login cannot access normal screens/APIs until a valid replacement is saved; expired restricted sessions lose access.
- **AC-03:** Password policy boundaries, exact confirmation/current-password checks, unique salts and hashed storage are enforced; replacement rotates session and revokes other sessions.
...
- **AC-05:** Logout invalidates the server token, clears UI account state, and prevents direct API/screen access and replay.
...
- **AC-15:** Claim/assign/reassign/unassign allow only eligible current owners and reject another owner's claim, terminal mutation and concurrent/stale writes.
...
- **AC-19:** Public Comments validate boundaries, author/time and append-only/idempotent behavior; owned Requesters and operational roles read them in deterministic order.
...
- **AC-21:** Public/Private composers are visibly separate; text/script-like content renders inert and failed posts preserve non-secret drafts without duplicates.
...
- **AC-24:** Deactivation/demotion/reset/account edits revoke affected sessions and unassign owners losing eligibility, without removing historical ownership/authorship.
...
- **AC-31:** Complete isolated unit/API/PostgreSQL/UI/regression/E2E execution and visual evidence are traceable to the final main revision, with no required skips or fabricated results.
...
Project decisions: opaque database sessions and built-in scrypt for the existing Node 22/Windows stack; password policy; explicit Administrator operational permissions; staff-wide collaboration; exact transition matrix; version concurrency and conversation request keys; public resolution summary/reasons; CLI one-time migrated-password delivery; no admin pagination; isolated test database. They may be amended through a documented contract change before dependent implementation, not silently weakened to match code.
