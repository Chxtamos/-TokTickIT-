...
- Normal session: absolute 8 hours and idle 30 minutes. Set expiresAt=createdAt+8 hours; a valid normal-session request may advance lastSeenAt but never expiresAt, and the session is invalid when `now >= expiresAt` or `now - lastSeenAt >= 30 minutes`. Initial-password restricted session: set createdAt=lastSeenAt and expiresAt=createdAt+15 minutes; validity is only `now < expiresAt`, activity never changes lastSeenAt/expiresAt, and there is no separate idle window. Cookie Max-Age matches the appropriate fixed absolute duration.
...
- Login/password-change rotate the session token and CSRF token. Password change follows the atomic transaction/recovery sequence below. Account edits/reset delete target sessions transactionally. Use `Cache-Control: no-store` on auth/user/Ticket/conversation responses and downloads. Password-bearing routes have bounded JSON size 16 KiB; other JSON routes 64 KiB.
...
| POST /auth/change-password | `{currentPassword:string,newPassword:string,confirmPassword:string}` plus cookie/Origin/CSRF | 200 AuthResponse with mustChangePassword=false and fresh cookie after the atomic transaction commits | 400 VALIDATION_FAILED for new policy/confirmation/same password; 401 CURRENT_PASSWORD_INVALID or SESSION_REQUIRED; 403 CSRF_INVALID; 409 VERSION_CONFLICT; safe 500 with full rollback |
...
Password-change atomicity and recovery:

1. Validate Origin/CSRF/body, verify current password against a captured User/session snapshot, derive the proposed password hash and generate raw replacement token/CSRF values before opening the database transaction. Raw values remain process memory only.
2. In one PostgreSQL transaction, lock/re-read the active User and authenticated Session and require their version/password/session state to equal the verified snapshot; a concurrent mismatch returns `409 VERSION_CONFLICT` with no mutation. Update passwordHash, mustChangePassword=false, passwordChangedAt, User.version/updatedAt; delete all of that User's sessions; insert one normal replacement Session with the token hash, CSRF value, createdAt=lastSeenAt=backend now and expiresAt=createdAt+8 hours.
3. A failure in update, session deletion or replacement insertion rolls the transaction back completely. The old password/session set remains valid and the endpoint returns a safe failure; it must not report success or clear the current cookie.
4. After commit, emit Set-Cookie and the 200 AuthResponse. If connection/response delivery fails after commit, the committed new password and replacement session remain authoritative and all old sessions stay revoked. The client clears uncertain local state and recovers by returning to Login with the new password; retrying with the old password cannot undo the committed change.
...
| PATCH /staff/tickets/:ticketId/owner | `{ticketOwnerId:number,expectedVersion:number}` | 200 TicketDetail; assigns/reassigns eligible owner; identical owner is unchanged | 400 VALIDATION_FAILED for null/manual-unassign or malformed ID; 400 ASSIGNEE_INVALID for unknown/inactive/ineligible target; 409 VERSION_CONFLICT/TICKET_TERMINAL |
...
- Owner, priority and indication no-op still validate expectedVersion and target eligibility/status first; no-op does not increment version. Null/manual-unassign owner requests are `400`; only account-ineligibility cleanup may clear an owner. Status same-state always 409. Validate unsupported resolutionSummary/reason on other transition targets as 400, not ignored. Staff owner/priority mutations on CLOSED/CANCELLED remain 409; Admin account deactivation/demotion performs internal eligibility cleanup on all currently assigned Tickets, including terminal Tickets, as specified in BR-23. That cleanup appends TicketOwnerChange before clearing ticketOwnerId, increments Ticket.version and preserves status/resolution fields.
- Mutation transactions check current role/activation, Ticket version, status and assignee eligibility, then update atomically. Initial assignment appends reason ASSIGNED; changed non-null owner appends REASSIGNED; account cleanup appends ACCOUNT_INELIGIBLE with null next owner. Each row records previous/next owner, actor and timestamp in the same transaction; no-op writes do not. Admin demotion/deactivation serializes with assignment eligibility checks to prevent newly assigning an ineligible owner. Priority/owner/status/indication writes increment version and updatedAt when changed.
