...
Version 1.0, planned 2026-09-15 before Lab 3 implementation. Rows remain **Planned / Not run** until an implementation PR records reproducible evidence; rows marked below have reproducible evidence from PR #70 or PR #71. A marked row records the executed coverage while any unlisted parameterized cases remain part of the planned matrix.
...
| API-02 | API | AC-02 | Restricted session allows only me/change/logout; issued t0 is valid at t0+15m-epsilon, activity at t0+14m59s changes neither lastSeenAt nor expiresAt, and me/change return401 at exactly/after t0+15m while logout204 | server/tests/lab-03/auth.api.test.ts | Passed fixed-expiry/lastSeen boundary cases in Server CI run [35236849889](https://github.com/Chxtamos/-TokTickIT-/actions/runs/35236849889) |
| API-03 | API/PostgreSQL | AC-03 | Wrong current password401 and invalid new/confirmation/same400; fault injection at User update/session delete/replacement insert proves full rollback; success atomically commits password+revocation+one normal session before cookie, and simulated lost response recovers by new-password login | server/tests/lab-03/auth.api.test.ts, server/tests/lab-03/auth.postgres.integration.test.ts | Passed covered rotation/rollback-boundary cases in Server CI run [35236849889](https://github.com/Chxtamos/-TokTickIT-/actions/runs/35236849889) |
...
| API-10 | API | AC-27 | Fifth failed pair/thirtieth IP failure allowed; next429/Retry-After; clock expiry/reset and process-restart bucket reset documented for bounded hashed in-memory throttle | server/tests/lab-03/auth.api.test.ts | Passed pair-limit/Retry-After case in Server CI run [35236849889](https://github.com/Chxtamos/-TokTickIT-/actions/runs/35236849889); remaining IP-window cases planned |
| API-11 | API | AC-27 | Cookie path/HttpOnly/SameSite/local Secure exception, HTTPS Secure, token rotation, absolute8h/idle30m expiry | server/tests/lab-03/auth.api.test.ts, server/tests/lab-03/auth.postgres.integration.test.ts | Passed cookie/rotation/idle cases in Server CI run [35236849889](https://github.com/Chxtamos/-TokTickIT-/actions/runs/35236849889); HTTPS-specific case planned |
...
| API-34 | API | AC-28 | Invalid JSON/bounded payloads and injected DB/file/service failures use exact safe errors/correlationId; no sensitive fragments | server/tests/lab-03/auth.api.test.ts | Passed malformed-JSON/auth safe-error case in Server CI run [35236849889](https://github.com/Chxtamos/-TokTickIT-/actions/runs/35236849889); remaining service-failure cases planned |
