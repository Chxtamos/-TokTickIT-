...
- User menu provides Change Password and Logout. Restricted sessions expose only Change Password and Logout. A restricted session expires exactly 15 minutes after login regardless of activity; the client may show remaining time for feedback but cannot extend it. At expiry, clear protected state and return to Login after the API's 401. No Development Requester selector, Change Requester or persisted requester identity.
...
Change Password: explain mandatory first-login requirement and fixed 15-minute issuance deadline when applicable; Current Password, New Password, Confirm New Password; plain policy hint (15-128 characters, spaces/Unicode allowed, max512 UTF-8 bytes, not whitespace-only); match validation; autocomplete=current-password/new-password; Save Password. Mandatory mode has no skip/normal-navigation control. Successful 200 clears secrets and opens the role landing screen with the committed replacement session; voluntary mode may cancel back to its landing screen. A pre-commit failure preserves the old credential/session and shows Retry. An ambiguous post-commit network failure clears uncertain local auth and directs the user to Login using the new password. Restricted-session expiry returns to Login and activity never extends it. Do not render the sample composition checklist as an instructor requirement.
...
Reuse Ticket/Attachment groups from Lab 2. Top: official number, status and Back to Queue preserving the same account's query. Submitted information (Requester, date, category/system, summary/description, Requested Priority) stays read-only. Operational area: owner selection, Claim when unassigned, Assign/Reassign confirmation, editable IT Priority and only permitted next statuses. There is no Staff manual-unassign action in Lab 3.
