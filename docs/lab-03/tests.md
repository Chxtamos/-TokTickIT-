...
| API-06 | API | AC-06 | Parameterize every protected endpoint/method against every role, including restricted/anonymous; matrix denial precedes resource lookup | server/tests/lab-03/authorization.api.test.ts | Passed locally on PR #72 with every currently implemented protected resource endpoint/method parameterized across Requester, IT Staff, Administrator, restricted sessions for each role, and anonymous requests; CI evidence pending this revision |
| API-07 | API | AC-06 | A cookie plus B requester header cannot impersonate B; requesterId body/query400; non-owner resource safe404/no bytes | server/tests/lab-03/authorization.api.test.ts | Passed locally on PR #72 for spoofed header/body/query identity, owner isolation, and fail-closed legacy adapter runtime guard; CI evidence pending this revision |
