...
| API-18 | API/PostgreSQL | AC-12 | Shared queue search/each combined filter, mine/unassigned/specific owner, sort priority/ties and accurate pagination | server/tests/lab-03/staff-queue.api.test.ts | Mock/API coverage passed locally 2026-09-19; 2 real PostgreSQL tests are implemented but skipped because the required isolated `TEST_DATABASE_URL`/`RUN_DB_INTEGRATION=1` were not configured. Hosted CI remains pending until push. |
...
| E2E-03 | E2E | AC-12, AC-13, AC-14, AC-15, AC-16, AC-17, AC-20, AC-21 | Real queue -> claim/reassign/priority/status/public/private/file download; Requester cannot read private note | client/e2e/lab-03/staff-ticket-flow.spec.ts | Full E2E-03 remains Planned / Not run because #60/#61/#62/#75 are outside #59. A focused Queue-only browser spec is implemented at `client/e2e/lab-03/staff-queue.spec.ts`; local execution remains pending an isolated database/browser environment and is not claimed as E2E-03. |
...
| Hosted CI | Pending until the user pushes this uncommitted branch. |
...
| #59 | Staff Queue API/UI | UNIT-04, API-19 and UI-05 passed locally; API-18 mock/API portion passed while its implemented PostgreSQL cases await isolated CI; focused Queue-only Playwright spec listed locally, not executed or claimed as full E2E-03 |
