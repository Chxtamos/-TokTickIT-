...
- Logout is busy until response; success or server 401 clears local auth state. If the logout request fails through network/500, keep the session state and show Retry so UI does not claim server invalidation that has not occurred.
...
- Each save uses the loaded expectedVersion. Do not let separate operational saves race within the same screen; after success replace authoritative detail/version. 409 offers Refresh and retains an unsaved non-secret draft for deliberate re-entry.
...
- Both conversations show deterministic author/time/content, empty/loading/failure/retry. Plain-text content wraps, preserves line breaks and cannot execute HTML. No edit/delete menu. Busy/retry reuses the same clientRequestId until success; a new draft uses a new key.
