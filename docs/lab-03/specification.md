...
- **FR-12:** Claim, assign, reassign or unassign one primary Ticket Owner using validated active eligible accounts and concurrency checks; retain owner-change provenance when an account loses eligibility.
...
- **BR-11:** Normal sessions expire after 8 hours absolute or 30 minutes idle, whichever occurs first. Login rotates tokens; logout deletes the current session if present and always clears the cookie. Repeated logout, including absent/expired sessions, succeeds with empty `204` so it is idempotent. A valid active session still requires its CSRF token; an absent/expired session has no token to validate. Password change/reset deletes all target sessions. No remember-me feature.
...
- **BR-22:** There is at most one primary Ticket Owner. Assignment targets must currently be active IT Staff or Administrator; unassignment uses null. Claim assigns self only when unassigned; claiming a Ticket already owned by self is a no-op; another owner's claim is `409 OWNER_CONFLICT`. Claim does not change status automatically. Every changed owner assignment/unassignment appends TicketOwnerChange in the same transaction; a no-op does not append an event.
- **BR-23:** User-driven deactivation/demotion revokes sessions and unassigns every Ticket whose current owner loses eligibility, including CLOSED/CANCELLED. This automatic cleanup is a separate account-safety operation, not a staff owner-mutation endpoint; staff claim/assign/unassign and IT Priority remain rejected on CLOSED/CANCELLED. The cleanup records a TicketOwnerChange row with previous owner, null next owner, actor and timestamp before clearing ticketOwnerId. Former owner attribution survives in this append-only record; requester/author/remover FKs also remain intact. Closed/Cancelled status and public resolution fields do not change. Reopening may then assign a new eligible owner through normal rules.
...
| TicketOwnerChange | id Int autoincrement PK; ticketId Int FK Ticket NOT NULL; previousOwnerId Int nullable FK User; nextOwnerId Int nullable FK User; actorId Int FK User NOT NULL (Staff/Admin who assigned or Admin who edited the account); changedAt DateTime NOT NULL default now; reason TicketOwnerChangeReason enum NOT NULL (`ASSIGNED`, `UNASSIGNED`, `ACCOUNT_INELIGIBLE`). Append-only provenance, not a new UI screen or full account-history feature. |
...
- **AC-15:** Claim/assign/reassign/unassign allow only eligible current owners and reject another owner's claim, terminal staff mutation and concurrent/stale writes; account-driven cleanup of terminal/current owners clears eligibility while preserving append-only owner attribution.
